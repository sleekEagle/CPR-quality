...
def detect_kypts_mp():
    ...
    for subj_dir in subj_dirs:
        ...
        for session_dir in session_dirs:
            print(session_dir)
            meanbb_path=os.path.join(session_dir,'mean_person_bbx.txt')
            if os.path.exists(meanbb_path):
                with open(meanbb_path, 'r') as file:
                    lines = file.readlines()
                    vals=lines[1].strip().split(',')
                    int_vals=[int(val) for val in vals]
                    mean_x,mean_y,mean_width,mean_height=int_vals
            ...
            with open(destination_file,'w') as file:
                ...
                for img_file in img_files:
                    ...
                    y_start=mean_y+int(mean_height*0.3)
                    x_start=mean_x
                    img=img[mean_y+int(mean_height*0.3):mean_y+mean_height,mean_x:mean_x+mean_width]
                    image,xy_vals=wrst.get_kypts(img)
                    if len(xy_vals)==0:
                        output[img_file.split('.')[0]]={}
                        continue
                    assert len(xy_vals)==21, f'Number of key points is not 21 for {img_file}'
                    
                    x_vals=[val[0]+x_start for val in xy_vals]
                    y_vals=[val[1]+y_start for val in xy_vals]
                    points=[]
